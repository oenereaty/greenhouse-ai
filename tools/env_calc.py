...
def sky_info(sky: str | None, pty: str | None = None, time: str | None = None) -> tuple[str, str]:
    """(하늘상태 표시, 예보 광량 추정) 반환. 광량은 KMA에 직접값이 없어 하늘상태 기반 추정.

    광량은 하늘상태(운량)만으로 추정하면 해가 없는 야간·새벽에도 '맑음=강'으로
    나오는 문제가 있다(사용자 피드백: 새벽인데 광량 강). time(HHMM)이 주어지면
    일출 전·일몰 후 시간대는 태양광 자체가 없으므로 광량을 '—'로 내린다. 하늘상태
    라벨(맑음 등)은 기상학적으로 야간에도 유효하므로 그대로 두되, 태양(☀️) 아이콘은
    야간에 뜨면 "새벽인데 해 아이콘"으로 오해를 사(사용자 피드백, 2026-07-10)
    달 아이콘(🌙)으로 바꿔 광량이 없다는 것을 시각적으로도 드러낸다.
    """
    lbl, icon, light = _SKY_LABEL.get(str(sky), ("—", "", "—"))
    p = _PTY_LABEL.get(str(pty), "")
    if time is not None and str(time).isdigit():
        hour = int(str(time).zfill(4)[:2])
        if hour < 6 or hour >= 19:  # 야간·새벽: 일사 없음 → 예보광량 없음
            light = "—"
            if icon == "☀️":
                icon = "🌙"
...
def env_interpret(temp: float, rh: float, vpd: float, co2: int, solar: float,
    ...
    # ASABE(2015) 인용 원문은 "적정 RH 60~90%, 90% 초과부터 위험"이다. 예전 코드는
    # 별도 인용인 "농진청 고온기(여름철 한정) 관리 기준 60~80%"를 연중 상시 경고
    # 임계값으로 잘못 섞어 써서 80~90%를 통째로 "다소 과습"(warn)으로 표시했다 —
    # 야간엔 온도가 낮아 RH가 85~91%대로 올라가는 게 일상적이라 매일 밤 경고가
    # 뜨는 비현실적인 결과가 났다(사용자 피드백, 2026-07-10). 인용 근거 그대로
    # 90%를 기준으로 되돌린다.
    if rh >= 90:
